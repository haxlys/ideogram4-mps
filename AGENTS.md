...
`backend` restarts FastAPI, starts the model daemon if it is not reachable, and
keeps WebUI running. `client` restarts only Vite. `full` restarts model daemon,
FastAPI, and WebUI. `doctor` checks local dependencies, model files, LLM paths,
ports, and memory policy.
...
- No backend test suite exists. Use compileall, WebUI test/lint/build, and
  manual smoke checks.
...
- Local LoRA files under `IDEOGRAM4_LORA_DIR` are exposed as UI presets via
  `/api/lora/presets`. `/api/lora/download` remains as a compatibility task
  endpoint, but the current MLX runtime rejects downloads because only local
  mflux-compatible `.safetensors` files are supported. Apply/remove operations
  are asynchronous operations polled through `/api/lora/operation/{task_id}`.
...
WebUI form state uses `useReducer` and controlled inputs. Do not add
`react-hook-form`, `zod`, or `@hookform/resolvers` unless a new feature
actually needs them.
